...
SBOMs are no longer a best practice or a DevSecOps nice-to-have. <br/>
**Article 13** of the CRA requires manufacturers to identify and document <br/>
every component in their product, in a machine-readable format, updated continuously.
```
grype python:latest
```

A simple vulnerability scan will certainly show us a list of software dependencies and their known vlnerabilities. <br/>
But this only scrapes the surface of what information is accessible within a Software Bill of Material.
...
As already stated, an SBOM is a JSON file. It provides a machine-readable inventory of software <br/>
components, dependencies, and metadata, commonly structured in **CycloneDX** or **SPDX** formats.

```
grype python:latest -o cyclonedx-json > python_latest.json
```

These files are generated by tools like Syft, Trivy, or AWS Inspector to track licenses, vulnerabilities, and supply chain security.<br/> 
In our case, we will use **Syft** to generate the JSON-formatted SBOM using OWASP's **CycloneDX** format.

```
cat python_latest.json
```
```
head -n 120 python_latest.json
```

Since the SBOM is simply too big to read entirely, we will push it to OWASP **dependency-track** <br/>
to get a full picture of our web application in that specific moment in time.
...
Since open-source dependencies change all the time, versioning, vulnerabilities, and transitive dependency relationships, <br/>
SBOMs should realistically be created as part of the software build process in order to fully comply with the Cyber Resilience Act (CRA).



If you have **[cosign](https://formulae.brew.sh/formula/cosign)** installed, you can pull the official, high-fidelity SBOM directly from their registry.<br/> 
...
A new CVE drops. The threat intelligence changes. The SBOM changes — even if the code didn't.<br/> 
Every build, every package update, every new vulnerability disclosure creates a new SBOM state.<br/> 
It's not a one-time document; it's a continuous record that has to stay current.<br/>
<br/>
**Article 14** applies from September 2026 (this year as I'm recording). <br/>
When a vulnerability is actively exploited in a product you ship, you have 24 hours to notify ENISA.<br/> 
That clock starts the moment you become aware of a compromise in your stack.<br/> 
<br/>
If your SBOM is out of date, incomplete, or locked in a CLI you have to query manually — you're simply guessing at this point.<br/> 
