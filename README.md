...
```
Scanning a known, flawed docker image with a long list of vulnerabilities.
```
...
This automatically creates the alpine project for our **alpine** ```container``` SBOM
...
We are going to use the freely-avilable **[python](https://images.chainguard.dev/directory/image/python/versions)** base image provided by Chainguard.
```
...
There was an error trying to pull licensed chaingaurd images without being a customer. <br/>
For the purpose of a free developer workshop, we'll stick with the freely-avaialable python base image:
...
SBOMs are no longer a best practice or a DevSecOps nice-to-have. **Article 13** of the CRA requires manufacturers to identify and document every component in their product, in a machine-readable format, updated continuously.
```
grype python:latest
```

A simple vulnerability scan will certainly show us a list of software dependencies and their known vlnerabilities. But this only scrapes the surface of what information is accessible within a Software Bill of Material.

```
grype python:latest --only-fixed
```

Dozens of direct software dependencies within this Python container image, each pulling in their own transitive dependnecies. 
...
As already stated, an SBOM is a JSON file. It provides a machine-readable inventory of software components, dependencies, and metadata, commonly structured in **CycloneDX** or **SPDX** formats.

```
grype python:latest -o cyclonedx-json > python_latest.json
```

These files are generated by tools like Syft, Trivy, or AWS Inspector to track licenses, vulnerabilities, and supply chain security. 
In our case, we will use **Syft** to generate the JSON-formatted SBOM using OWASP's **CycloneDX** format.

<br/><br/>

Since the SBOM is simply too big to read entirely, we will push it to OWASP **dependency-track** to get a full picture of our web application in that specific moment in time.
...
curl -X "POST" "http://localhost:8081/api/v1/bom" \
     ...
     -H "X-Api-Key: $DTRACK_API_KEY" \
     -F "autoCreate=true" \
     -F "projectName=python-latest" \
     -F "projectVersion=latest" \
     -F "bom=@python_latest.json"
```

Since open-source dependencies change all the time, versioning, vulnerabilities, and transitive dependency relationships, SBOMs should realistically be created as part of the software build process in order to fully comply with the Cyber Resilience Act (CRA).

<br/><br/>

If you have **[cosign](https://formulae.brew.sh/formula/cosign)** installed, you can pull the official, high-fidelity SBOM directly from their registry. 
This is much more accurate than a Grype scan:
```
cosign download attestation \
  --platform=linux/arm64 \
  --predicate-type=https://spdx.dev/Document \
...
A new CVE drops. The threat intelligence changes. The SBOM changes — even if the code didn't. 
Every build, every package update, every new vulnerability disclosure creates a new SBOM state. 
It's not a one-time document; it's a continuous record that has to stay current.
<br/>
**Article 14** applies from September 2026 (this year as I'm recording). 
When a vulnerability is actively exploited in a product you ship, you have 24 hours to notify ENISA. 
That clock starts the moment you become aware of a compromise in your stack. 
<br/>
If your SBOM is out of date, incomplete, or locked in a CLI you have to query manually — you're simply guessing at this point. 
The question isn't whether SBOMs matter. It's whether yours is ready when the clock starts.


```
grype python:latest -o json | grype explain --id CVE-2025-15366
```
